...
const CartPage = ({ cart, setCart, setPage }) => {
  ...
  if (ordered) return (
    <div style={{ padding:24, display:"flex", alignItems:"center", justifyContent:"center", minHeight:400 }}>
      <div className="scale-in" style={{ textAlign:"center" }}>
        ...
        <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:28, color:"#D4A853", marginBottom:8 }}>Order Placed!</h2>
  ...
  return (
    <div style={{ padding:24 }}>
      <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:26, color:"#E8DCC8", marginBottom:24 }}>Shopping Cart</h2>
      <div style={{ display:"grid", gridTemplateColumns:"3fr 2fr", gap:24 }}>
            ...
            <div key={`${item.id}-${item.type}`} className="stat-card card-hover fade-up"
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                  <div>
                    <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:15, color:"#E8DCC8", marginBottom:4 }}>{item.name}</h3>
                    <p style={{ fontSize:11, color:"#8A7A60", marginBottom:6 }}>{item.size} · {item.material}</p>
                    <span style={{ background:item.type==="wholesale"?"rgba(34,197,94,.15)":"rgba(212,168,83,.15)", color:item.type==="wholesale"?"#22C55E":"#D4A853", padding:"2px 10px", borderRadius:20, fontSize:10, fontWeight:700 }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, background:"#141210", borderRadius:8, padding:"4px 8px" }}>
                    <button onClick={()=>updateQty(item.id,item.type,-1)} style={{ background:"none", border:"none", color:"#D4A853", cursor:"pointer", fontSize:18, lineHeight:1 }}>−</button>
                    <span style={{ fontSize:14, fontWeight:600, color:"#E8DCC8", minWidth:24, textAlign:"center" }}>{item.qty}</span>
                    <button onClick={()=>updateQty(item.id,item.type,1)} style={{ background:"none", border:"none", color:"#D4A853", cursor:"pointer", fontSize:18, lineHeight:1 }}>+</button>
                  </div>
                  <div style={{ textAlign:"right" }}>
                    <p style={{ fontSize:11, color:"#8A7A60" }}>₹{item.price} × {item.qty}</p>
                    <p style={{ fontSize:16, fontWeight:700, color:"#D4A853" }}>₹{(parseFloat(item.price)*item.qty).toFixed(2)}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div>
          <div className="stat-card" style={{ borderRadius:16, padding:24 }}>
            <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:17, color:"#E8DCC8", marginBottom:20 }}>Order Summary</h3>
            <div style={{ display:"flex", flexDirection:"column", gap:12, marginBottom:20 }}>
              {[["Subtotal",`₹${subtotal.toFixed(2)}`],["GST (18%)",`₹${tax.toFixed(2)}`]].map(([k,v])=>(
                <div key={k} style={{ display:"flex", justifyContent:"space-between" }}>
                  ...
                  <span style={{ fontSize:13, color:"#C8B898" }}>{v}</span>
                </div>
              ))}
              <div style={{ borderTop:"1px solid #2A2418", paddingTop:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <span style={{ fontWeight:600, color:"#E8DCC8" }}>Total</span>
                <span style={{ fontSize:22, fontWeight:700, color:"#D4A853", fontFamily:"'Playfair Display',serif" }}>₹{total.toFixed(2)}</span>
              </div>
            </div>
            ...
            {!showPayment ? (
              ...
              <div>
                ...
                <h4 style={{ color:"#D4A853", fontSize:13, fontWeight:600, marginTop:20, marginBottom:12, textTransform:"uppercase", letterSpacing:".08em" }}>Payment Method</h4>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:20 }}>
                  {["UPI","Card","Cash","Online"].map(m=>(
                    <div key={m} className={`payment-option ${payMethod===m?"selected":""}`} onClick={()=>setPayMethod(m)} style={{ textAlign:"center" }}>
                      <p style={{ fontSize:13, fontWeight:600, color:payMethod===m?"#D4A853":"#8A7A60" }}>{m}</p>
                    </div>
                  ))}
                </div>
                <div style={{ background:"rgba(212,168,83,.05)", border:"1px solid rgba(212,168,83,.1)", borderRadius:10, padding:12, marginBottom:16 }}>
                  <p style={{ fontSize:11, color:"#8A7A60", marginBottom:8 }}>{cart.length} item(s) · via {payMethod}</p>
                  {cart.map(item=>(
                    <div key={`${item.id}-${item.type}`} style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"#C8B898", marginBottom:4 }}>
                      <span>{item.name} ({item.type}) ×{item.qty}</span>
                      <span style={{ color:"#D4A853" }}>₹{(parseFloat(item.price)*item.qty).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
